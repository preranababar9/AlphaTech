export const detail = [
        {
                img : "/images/hero.webp",
                title : "iphone 15 Pro",
                subtitle : "Beyond Pro: Experience Perfection"
        },
        {
                img : "/images/slider2.webp",
                title : "Microsoft Surface Pro 4",
                subtitle : "Beyond Pro: Experience Perfection"
        },
        {
                img : "/images/slider3.webp",
                title : "Samsung Galaxy S23 Ultra ",
                subtitle : "Beyond Pro: Experience Perfection"
        },
        
        {
                img : "/images/slider4.webp",
                title : "MacBook Pro 13",
                subtitle : "Beyond Pro: Experience Perfection"
        },
        
        {
                img : "/images/slider5.webp",
                title : "Samsung Q80B 4K",
                subtitle : "Beyond Pro: Experience Perfection"
        },
        
        

]

//feature data 
export const info = [
        {
                icon : "/icons/delivery.svg",
                name : "Free Shipping",
                msg: "Order above $200"
        },
        {
                icon : "/icons/money.svg",
                name: "Discount",
                msg: "Every week new sales"
        },
        {
                icon : "/icons/lock.svg",
                name: "Secure Payments",
                msg: "Secured by Stripe"
        },
        {
                icon : "/icons/call.svg",
                name: "24/7 Support",
                msg: "Phone and Email Support"
        },
]

 export const product = [
        {
                name : "Samsung 65",
                des : "Class QLED 4K Q80C",
                img: "/images/product1.png",
        },
        {
                name : "Macbook Pro 2019",
                des : "16 inch 16gb Ram 512gb SSD Intel core I7",
                img: "/images/product2.png",
        },
        {
                name : "Apple watch Series 6",
                des : "(GPS, 40mm) - Gold Aluminum Case with Pink Sand Sport Band",
                img: "/images/product3.png",
        },
        {
                name : "ipad Pro",
                des : "11 64gb silver",
                img: "/images/product4.png",
        },
        {
                name : "HP EliteBook",
                des : "830 G8 13.3 Notebook - Full HD - 1920 x 1080 - Intel Core i7 11th Gen",
                img: "/images/product5.png",
        },
        {
                name : "iPhone 11 pro",
                des : "256 gb Gray",
                img: "/images/product6.png",
        },
        {
                name : "Samsung Galaxy S23 Ultra",
                des : "5G Dual SIM 256GB ROM 8GB RAM",
                img: "/images/slider3.webp",
        },
        {
                name : "Samsung Monitor",
                des : "28 UHD Monitor",
                img: "/images/product8.png",
        },
        {
                name : "Lidnio Power Bank",
                des : "Magnetic Wireless Fast Charging Power Bank 20000MAH",
                img: "/images/product9.png",
        },
        {
                name : "Laptop Bag",
                des : "Shona Laptop Bags",
                img: "/images/product10.png",
        },
      
      
]

// sustaniable tech data

export const tech = [
        {
                num :" 01",
                title: "Reducing E-Waste",
                about : "The electronics industry generates substantial e-waste annually, containing hazardous materials harmful to the environment. Choosing certified used technology extends device lifespans, reduces the need for new manufacturing, and lowers e-waste production, thereby minimizing the environmental impact of device disposal.",
                img : "/images/tech1.png"
        },
        {
                num : "02",
                title: "Resource Conservation",
                about : "The production of electronic devices consumes valuable natural resources. By choosing certified used technology, consumers conserve these resources, as refurbishment uses fewer raw materials than manufacturing new devices. This conservation is crucial for preserving the environment.",
                   img : "/images/tech2.png"
        },
        {
                num : "02",
                title: "Resource Conservation",
                about : "The production of electronic devices consumes valuable natural resources. By choosing certified used technology, consumers conserve these resources, as refurbishment uses fewer raw materials than manufacturing new devices. This conservation is crucial for preserving the environment.",
                   img : "/images/tech2.png"
        },
]

export const reviews =[
        {
                review : "“I loved visiting the website to get a gauge of the laptops I liked, and when I visited the store I fell in love with it. The laptop was in pristine condition and got a great deal! I highly recommend”",
                name :"Muhammad"
        },
        {
                review : "“I'm thrilled with the laptop I won from the Alpha Tech giveaway. Its performance is outstanding - super fast, with incredible graphics and clear audio. The battery life is remarkable, lasting all day. I use it constantly and would recommend...”",
                name: "Raina"
        },
        {
                review : "“We needed a 65 inch 4k Smart TV, and we saved over 1,000,000 tsh with AlphaTech! The experience was great as we were able to test it out within the store and purchased top quality at affordable rates!”",
                name: "Sumaiya"
        },

]